...
[data-testid="metric-container"] { background: #ffffff; border: 1px solid #e8ecf2; border-radius: 16px; padding: 20px 24px !important; box-shadow: 0 1px 4px rgba(0,0,0,0.05); transition: box-shadow .2s, border-color .2s; }
...
.refresh-info { background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:10px 16px;font-size:12px;color:#6b7280;margin-top:8px; }
...
hr { border-color: #e8ecf2 !important; }
.ratio-pill { display:inline-block;background:linear-gradient(135deg,#fef9ee,#fef3c7);border:1px solid #fde68a;border-radius:8px;padding:3px 10px;font-size:11px;color:#92400e;font-weight:600; }
...
def apply_layout(fig, title="", height=320, show_legend=True):
    fig.update_layout(**PLOTLY_LAYOUT, title=dict(text=title, font=dict(size=14, color="#111827"), x=0), height=height, showlegend=show_legend)
...
def load_data():
    ...
    if "Production Date" in df.columns:
        ...
        if date_cols:
            ...
            raise KeyError(f"'Production Date' column not found. Available columns: {list(df.columns)}")
...
with st.sidebar:
    st.markdown(f"""
    <div class="live-badge"><div class="live-dot"></div> Live · Zoho Creator</div>
    <div class="refresh-info">
      🕐 Last fetched: <b>{fetched_at.strftime('%H:%M:%S') if fetched_at else '—'}</b><br>
      ⏱ Auto-refresh: every <b>{AUTO_REFRESH_SEC}s</b><br>
      📦 Total records: <b>{len(raw):,}</b>
    </div>
    """, unsafe_allow_html=True)
    ...
    date_range = st.date_input("date_range", value=(min_date, max_date), min_value=min_date, max_value=max_date, label_visibility="collapsed")
    st.markdown("**🏭 Party**")
...
# ═══════════════════════════════════════════════════════════
# ── NEW HELPER FUNCTIONS FOR BUSINESS DECISION SECTIONS ──
# ═══════════════════════════════════════════════════════════

def render_machine_utilization(data):
    """Section 1: Machine Utilization Analysis — NEW (M.No. was never used before)"""
...
def render_master_productivity(data):
    """Section 2: Master Productivity Dashboard — Replaces existing simple pie with full leaderboard"""
...
def render_small_lot_analysis(data):
    """Section 3: Small Lot Analysis — NEW"""
    ...
    for col, (_, row) in zip([c1, c2, c3], lot_counts.iterrows()):
        pill_cls = "lot-pill-small" if "Small" in row["Category"] else ("lot-pill-large" if "Large" in row["Category"] else "lot-pill-medium")
    ...
    with c_warn:
        ...
        if small_pct >= 40:
            insight_card(f"<strong>{small_pct:.1f}% of all lots are Small (&lt;100 kg).</strong> This is critically high — small lots increase machine setup time, labour cost per kg, and dye wastage. Consider lot consolidation or minimum lot policies.", kind="risk")
        elif small_pct >= 20:
            insight_card(f"<strong>{small_pct:.1f}% of lots are Small (&lt;100 kg)</strong>, moderately increasing operational overhead. Review if these lots can be batched or scheduled together.", kind="warn")
        else:
            insight_card(f"<strong>Small lot percentage is {small_pct:.1f}%</strong> — within acceptable range. Monitor to prevent creep.", kind="good")
        ...
        if not party_small.empty:
            insight_card(
                ...
                + "</strong>. Consider negotiating minimum batch sizes with these parties.",
                kind="info"
            )


def render_party_dependency(data):
    """Section 4: Party Dependency (Pareto) — Extends existing party bar chart with Pareto + risk"""
    ...
    with c1:
        ...
        fig_pareto.add_trace(go.Scatter(
            ...
            line=dict(color="#6366f1", width=2.5, dash="solid"),
    ...
    if not high_risk.empty:
        for _, r in high_risk.iterrows():
            insight_card(
                f"<strong>{r['Party Name']}</strong> contributes <strong>{r['Contribution %']:.1f}%</strong> of total production weight. "
                "This creates a <strong>high business dependency risk</strong> — a sudden order drop could significantly impact operations.",
                kind="risk"
            )
    parties_80 = int((party["Cumulative %"] <= 80).sum()) + 1
    insight_card(
        f"<strong>Top {parties_80} parties</strong> account for 80% of total production (Pareto principle). "
        "Diversifying the customer base beyond these parties will reduce operational risk.",
        kind="info"
    )


def render_chemical_consumption(data):
    """Section 5: Chemical Consumption Analysis — NEW"""
    chem_cols = [c for c in ["G/ACID", "DFT", "RC", "XNI", "SOFTNER", "MB"] if c in data.columns]
    if not chem_cols:
        st.info("ℹ️ No chemical consumption columns found in data.")
        return
    ...
    # Heatmap: Quality × Chemical
    st.markdown("#### Quality vs Chemical Consumption Heatmap")
    qual_chem = data.groupby("QUALITY")[chem_cols].sum()
    qual_chem = qual_chem.loc[qual_chem.sum(axis=1).nlargest(15).index]  # top 15 qualities

    fig_hm = go.Figure(go.Heatmap(
        z=qual_chem.values,
        x=chem_cols,
        y=qual_chem.index.tolist(),
        colorscale=[[0, "#f9fafb"], [0.5, "#fef3c7"], [1, "#d97706"]],
    ...
    insight_card(
        ...
        f"({top_chem['Total Used']:,.0f} units total, {top_chem['Per KG']:.4f} per kg). "
        "Prioritize cost negotiation or supplier evaluation for this chemical.",
        kind="info"
    )

    # Top quality by chemical consumption
    top_qual_by_chem = qual_chem.sum(axis=1).idxmax()
    insight_card(
        f"Quality <strong>{top_qual_by_chem}</strong> has the highest overall chemical consumption. "
        "Review pricing for this quality to ensure adequate margin coverage.",
        kind="warn"
    )


def render_shade_complexity(data):
    """Section 6: Shade Complexity Analysis — NEW"""
    chem_cols = [c for c in ["G/ACID", "DFT", "RC", "XNI", "SOFTNER", "MB"] if c in data.columns]
    if not chem_cols:
        st.info("ℹ️ Chemical columns not available for shade complexity analysis.")
        return
    ...
    with c2:
        display_cols_shade = ["SHADE"] + chem_cols + ["Total Avg Chem"]
        display_cols_shade = [c for c in display_cols_shade if c in shade_chem.columns]
        rows = ""
    ...
    insight_card(
        f"Shade <strong>{top_shade}</strong> has the highest average chemical consumption. "
        "This shade likely requires higher processing effort — consider applying a <strong>premium pricing</strong> surcharge.",
        kind="warn"
    )
    insight_card(
        "Shades in the top 10 complexity list consume significantly more chemicals per lot. "
        "Review job-costing models to ensure these are not processed at a loss.",
        kind="info"
    )


def render_quality_efficiency(data):
    """Section 7: Quality Efficiency (MTRS/KG) — NEW"""
    ...
    if not low_eff.empty:
        ...
        insight_card(
            f"Qualities <strong>{names}</strong> have a significantly <strong>low MTRS/KG ratio</strong> "
            "(below 75% of average). These may indicate heavy fabrics or data anomalies — worth investigating.",
            kind="risk"
        )
    ...
    if not high_eff.empty:
        ...
        insight_card(
            f"Qualities <strong>{names}</strong> have a notably <strong>high MTRS/KG ratio</strong> — "
            "these are lightweight fabrics and may require more careful handling per metre.",
            kind="info"
        )


def render_mom_comparison(data, sel_month):
    """Section 8: Month-over-Month Comparison — NEW"""
    ...
    if prev_df.empty:
        st.info(f"ℹ️ No data available for {prev_m.strftime('%B %Y')} to compare against.")
        return
    ...
    if prev_wt:
        ...
        if d > 5:
            insight_card(f"Production weight <strong>increased by {d:.1f}%</strong> vs {prev_m.strftime('%B %Y')}. Positive momentum — ensure machine and staffing capacity can sustain this.", kind="good")
        elif d < -5:
...
def render_business_insights(data, scope_label=""):
    """Section 9: Business Insights Panel — NEW (rule-based)"""
    ...
    for pname, wt in party.items():
        ...
        if pct > 40:
            insights.append(("risk", "🔴", f"<strong>{pname}</strong> contributes <strong>{pct:.1f}%</strong> of production weight. Business dependency risk is <strong>critically high</strong>."))
        elif pct > 30:
            insights.append(("warn", "⚠️", f"<strong>{pname}</strong> contributes <strong>{pct:.1f}%</strong> of production. Dependency risk is elevated — diversify order intake."))
    ...
    if "M.No." in data.columns:
        ...
        for mno, wt in mach_wt.items():
            if avg_mwt and wt < avg_mwt * 0.6:
                ...
                insights.append(("warn", "⚙️", f"Machine <strong>{mno}</strong> has processed <strong>{pct_below:.0f}% less</strong> than average machine production. Check for downtime, maintenance issues, or underallocation."))

    # Small lots
    small_pct = (data["Weight"] < 100).sum() / len(data) * 100 if len(data) else 0
    if small_pct > 30:
        insights.append(("risk", "📦", f"<strong>Small lots (&lt;100 kg)</strong> account for <strong>{small_pct:.1f}%</strong> of all lots — increasing operational costs per kg processed."))
    elif small_pct > 15:
        insights.append(("warn", "📦", f"Small lots represent <strong>{small_pct:.1f}%</strong> of total lots. Watch this metric — above 30% is operationally costly."))

    # Shade chemical complexity
    chem_cols = [c for c in ["G/ACID", "DFT", "RC", "XNI", "SOFTNER", "MB"] if c in data.columns]
    if chem_cols:
        shade_chem = data.groupby("SHADE")[chem_cols].mean().sum(axis=1)
        if not shade_chem.empty:
            top_shade = shade_chem.idxmax()
            insights.append(("info", "🎨", f"Shade <strong>{top_shade}</strong> has the highest average chemical consumption — consider premium pricing for this shade."))

    # Top master
    master_wt = data.groupby("MASTER NAME")["Weight"].sum()
    if not master_wt.empty:
        top_master = master_wt.idxmax()
        insights.append(("good", "👤", f"Master <strong>{top_master}</strong> supervised the highest production weight (<strong>{fmt_in(int(master_wt[top_master]))} kg</strong>). Recognize and retain top performers."))
    ...
    if total_days > 0:
        ...
        if utilization < 70:
            insights.append(("warn", "📅", f"Factory was operational for only <strong>{working_days} of {total_days} days</strong> ({utilization:.0f}% utilization). Review downtime causes."))

    # Quality efficiency outliers
    ...
    if not qual_eff.empty:
        ...
        if not low_q.empty:
            ...
            insights.append(("info", "📐", f"Qualities <strong>{names}</strong> have unusually low MTRS/KG ratios — verify data accuracy or review processing parameters."))

    if not insights:
        insights.append(("good", "✅", "No major risk signals detected in the current data. Operations appear within normal parameters."))
...
if current_view == "day":
    ...
    k1.metric("📏 Metres",       fmt_in(d_mtrs))
    k2.metric("⚖️ Weight (kg)",  fmt_in(d_weight))
    k3.metric("🧵 Lots",         d_lots)
    k4.metric("🏭 Parties",      d_parties)
    k5.metric("📐 Mtr/KG Ratio", f"{d_ratio}x")

    st.markdown(f"<div style='margin:6px 0 12px;'><span class='ratio-pill'>📐 Mtr ÷ KG: {d_ratio}x — every 1 kg = {d_ratio} metres</span></div>", unsafe_allow_html=True)
    st.divider()
    ...
    st.markdown("### 📋 Party-wise Summary — Today")
    ...
    # ── NEW: Business Decision Sections for Day View ──
    st.divider()
    st.markdown("## 🧠 Business Decision Analysis — Day View")
...
# ═══════════════════════════════════════════════════════════
    ...
    k1.metric("📏 Total Metres",      fmt_in(total_mtrs),   delta=delta_mtrs)
    k2.metric("⚖️ Total Weight (kg)", fmt_in(total_weight), delta=delta_weight)
    ...
    k6.metric("📈 Avg KG / Day",      fmt_in(avg_kg_per_day))

    st.markdown(f"<div style='margin:8px 0 4px;'><span class='ratio-pill'>📐 Mtr ÷ KG Ratio: {mtr_kg_ratio}x — every 1 kg = {mtr_kg_ratio} metres</span></div>", unsafe_allow_html=True)
    st.divider()
    ...
    st.plotly_chart(fig_wt_day, use_container_width=True)
    ...
    fig_party = go.Figure(go.Bar(
        ...
        text=[f"{fmt_in(v)} kg ({s}%)" for v,s in zip(party_df["Weight"], party_df["Share"])],
        textposition="outside", textfont=dict(color="#374151", size=11, family="JetBrains Mono"),
    ))
    apply_layout(fig_party, "Party-wise Production (KG)", height=420, show_legend=False)
    ...
    apply_layout(fig_qual, "Top Fabric Qualities by KG", height=420, show_legend=False)
    ...
    with st.expander("🧵 Quality-wise Breakdown"):
        ...
        with col_b:
            ...
            st.plotly_chart(fig_qtree, use_container_width=True)

    # ── NEW: Business Decision Sections for Month View ──
    st.divider()
    st.markdown("## 🧠 Business Decision Analysis — Month View")
    ...
    st.divider()
    render_mom_comparison(df, sel_month)   # uses full df for previous-month data
    st.divider()
