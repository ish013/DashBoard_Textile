...
def _load_lines_report(report_name):
    ...
    base_url = f"https://creator.zoho.com/api/v2/{owner}/{app}/report/{report_name}"
...
def load_data():
    ...
    # ── lightweight field snapshot (kept tiny; only surfaced in debug mode) ──
    ...
    if not (chem_rows or dye_rows):
        if lines_report:
            d_chem, d_dye = _load_lines_report(lines_report)
        else:
    ...
    canon = {
        ...
        "totalcost": "Total Cost", "lastupdated": "Last Updated",
    }
    ...
    for c in ["MTRS", "Weight", "Total Cost", "Total Chemical Cost", "Total Dye Cost",
...
# Optional developer-only debug panel (hidden from stakeholders).
# Enable by adding  debug = true  under [zoho_creator] in secrets.toml
if st.secrets["zoho_creator"].get("debug"):
    with st.expander("🔧 Debug — data load"):
        st.markdown(f"**Chemical line-items:** {len(raw_chem)}  ·  **Dye line-items:** {len(raw_dye)}  ·  "
                    f"**Production rows:** {len(raw)}")
        st.caption("First record fields: " + ", ".join(f"`{c}`" for c in diag.get("columns", [])))
        if not raw_chem.empty:
            st.markdown("**Chemical sample**"); st.dataframe(raw_chem.head(8), use_container_width=True)
        if not raw_dye.empty:
            st.markdown("**Dye sample**"); st.dataframe(raw_dye.head(8), use_container_width=True)
...
# ═══════════════════════════════════════════════════════════
    ...
    with k6: ratio_widget(mtr_kg_ratio)
    if HAS_COST:
    ...
    with c3:
        ...
        top_shades = shade_df.head(10).copy(); others = shade_df.iloc[10:]["Lots"].sum()
        if others > 0:
