...
[data-testid="stSidebar"] { background: #113941 !important; border-right: 1px solid #0d2d33 !important; box-shadow: 2px 0 12px rgba(0,0,0,0.04); }
[data-testid="stSidebar"] * { color: #DEEDEE !important; }
...
[data-testid="metric-container"]:hover { box-shadow: 0 4px 16px rgba(2,163,166,0.2); border-color: #02A3A6; }
...
h2, h3 { color: #113941 !important; font-weight: 700 !important; }
[data-testid="stPlotlyChart"] > div { border-radius: 16px !important; overflow: hidden; box-shadow: 0 1px 4px rgba(0,0,0,0.06); }
.stMultiSelect [data-baseweb="tag"] { background: #DEEDEE !important; color: #113941 !important; border-radius: 6px !important; }
...
.live-badge { display:inline-flex;align-items:center;gap:6px;background:#e6f7f7;border:1px solid #02A3A6;border-radius:20px;padding:5px 12px;font-size:12px;color:#02A3A6;font-weight:500; }
.live-dot { width:7px;height:7px;border-radius:50%;background:#22c55e;animation:pulse 2s infinite; }
@keyframes pulse { 0%,100%{opacity:1}50%{opacity:.3} }
.refresh-info { background:#0d2d33;border:1px solid #1a4a52;border-radius:10px;padding:10px 16px;font-size:12px;color:#DEEDEE;margin-top:8px; }
...
.sum-table th { background:#113941;color:#DEEDEE;text-transform:uppercase;font-size:11px;letter-spacing:.06em;padding:11px 16px;border-bottom:1px solid #1a4a52;text-align:left;font-weight:600; }
...
.sum-table td:first-child { font-family:'Inter',sans-serif;color:#02A3A6;font-weight:600; }
.sum-table tr:hover td { background:#e6f7f7; }
.ratio-pill { display:inline-block;background:linear-gradient(135deg,#e6f7f7,#DEEDEE);border:1px solid #02A3A6;border-radius:8px;padding:3px 10px;font-size:11px;color:#113941;font-weight:600; }
.prod-sheet-table { width:100%;border-collapse:collapse;font-family:'Inter',sans-serif;font-size:12px; }
.prod-sheet-table th { background:#113941;color:#ffffff;padding:8px 10px;text-align:center;font-size:11px;letter-spacing:.04em;border:1px solid #1a4a52; }
...
.prod-sheet-table tr:nth-child(even) td { background:#e6f7f7; }
.prod-sheet-table tr:hover td { background:#DEEDEE; }
.prod-sheet-total td { background:#113941 !important;color:#ffffff !important;font-weight:700; }
.day-header { background:linear-gradient(135deg,#113941,#02A3A6);color:#ffffff;border-radius:14px;padding:16px 24px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between; }
</style>
...
PLOTLY_LAYOUT = dict(
    paper_bgcolor="rgba(255,255,255,1)", plot_bgcolor="rgba(247,247,247,1)",
    font=dict(family="Inter", color="#4E5973", size=12),
    margin=dict(l=10, r=10, t=40, b=10),
    legend=dict(bgcolor="rgba(255,255,255,0.9)", bordercolor="#DEEDEE", borderwidth=1, font=dict(color="#113941", size=11)),
    xaxis=dict(gridcolor="#DEEDEE", zerolinecolor="#DEEDEE", linecolor="#DEEDEE"),
    yaxis=dict(gridcolor="#DEEDEE", zerolinecolor="#DEEDEE", linecolor="#DEEDEE"),
)
PALETTE = ["#02A3A6","#2384A1","#2E556B","#113941","#C44545","#4E5973","#DEEDEE","#222D35","#02A3A6","#2384A1","#2E556B","#4E5973","#C44545","#113941","#222D35"]

def apply_layout(fig, title="", height=320, show_legend=True):
    fig.update_layout(**PLOTLY_LAYOUT, title=dict(text=title, font=dict(size=14, color="#113941", weight=600), x=0), height=height, showlegend=show_legend)
...
def load_data():
    """Fetch ALL records from Zoho Creator report with pagination."""
    cfg         = st.secrets["zoho_creator"]
    ...
    all_records = []
    start_index = 0
    limit       = 200          # Zoho Creator max per page
    ...
    while True:
        ...
        # If fewer records than limit returned, we've reached the end
    ...
    # ── Step 1: Extract display_value from Zoho lookup/dropdown fields ──
    # Zoho Creator returns lookup & dropdown fields as dicts:
    #   {"display_value": "SHRUTI", "ID": "485831800000028004"}
    # We extract just the display_value string for all such columns.
    def extract_display(val):
        """Extract display_value from Zoho dict fields, or return value as-is."""
    ...
    # ── Step 2: Rename Zoho field link names → friendly display names ──
    col_map = {
        # Common Zoho Creator link name patterns:
    ...
    if "Production Date" in df.columns:
        ...
        if date_cols:
            ...
            raise KeyError(
                f"'Production Date' column not found. Available columns: {list(df.columns)}"
            )
...
<div style="display:flex;align-items:center;gap:16px;padding:8px 0 24px;">
  <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#113941,#02A3A6);
    display:flex;align-items:center;justify-content:center;font-size:24px;box-shadow:0 4px 14px rgba(2,163,166,0.35);">🎨</div>
  <div>
    <div style="font-size:1.55rem;font-weight:800;color:#113941;letter-spacing:-.5px;">Dyeing Operations Dashboard</div>
...
if current_view == "day":
    ...
    with dc1:
        ...
        fig_pd = go.Figure(go.Bar(
            ...
            marker=dict(color=p_day["Weight"], colorscale=[[0,"#DEEDEE"],[0.5,"#2384A1"],[1,"#113941"]], showscale=False, line=dict(width=0)),
            text=[f"{fmt_in(v)} kg ({s}%)" for v,s in zip(p_day["Weight"], p_day["Share"])],
            textposition="outside", textfont=dict(color="#113941", size=11, family="JetBrains Mono"),
        ))
    ...
    with dc2:
        ...
        fig_qd = go.Figure(go.Bar(
            ...
            marker=dict(color=list(range(len(q_day))), colorscale="Teal", showscale=False, line=dict(width=0)),
            text=[f"{fmt_in(v)}\n({s}%)" for v,s in zip(q_day["Weight"], q_day["Share"])],
            textposition="outside", textfont=dict(color="#113941", size=10, family="JetBrains Mono"),
        ))
    ...
    with dc3:
        ...
        fig_sd = go.Figure(go.Pie(
            ...
            marker=dict(colors=PALETTE[:len(shade_day)], line=dict(color="#F7F7F7", width=2)),
            textinfo="label+percent", textfont=dict(size=11, color="#113941"),
        ))
    ...
    with dc4:
        ...
        fig_md = go.Figure(go.Pie(
            ...
            marker=dict(colors=["#02A3A6","#2384A1","#113941","#2E556B","#C44545"], line=dict(color="#F7F7F7", width=2)),
            textinfo="label+percent", textfont=dict(size=12, color="#113941"),
        ))
...
# ═══════════════════════════════════════════════════════════
    ...
    bar_colors_wt = ["#02A3A6" if v >= avg_wt*1.1 else "#C44545" if v < avg_wt*0.7 else "#2384A1" for v in daily["Weight"]]
    fig_wt_day = go.Figure(go.Bar(
        ...
        textfont=dict(size=9, family="JetBrains Mono", color="#113941"),
    ))
    fig_wt_day.add_hline(y=avg_wt, line_dash="dot", line_color="#02A3A6", line_width=1.5,
                         annotation_text=f"Avg {fmt_in(int(avg_wt))} kg",
                         annotation_font_color="#113941", annotation_position="top right")
    ...
    with c1:
        ...
        fig_party = go.Figure(go.Bar(
            ...
            marker=dict(color=party_df["Weight"], colorscale=[[0,"#DEEDEE"],[0.5,"#2384A1"],[1,"#113941"]], showscale=False, line=dict(width=0)),
    ...
    with c2:
        ...
        fig_qual = go.Figure(go.Bar(
            ...
            marker=dict(color=list(range(len(quality_df))), colorscale="Teal", showscale=False, line=dict(width=0)),
    ...
    with c3:
        ...
        fig_shade = go.Figure(go.Pie(
            ...
            marker=dict(colors=PALETTE[:len(top_shades)], line=dict(color="#F7F7F7", width=2)),
            textinfo="label+percent", textfont=dict(size=11, color="#113941"),
        ))
    ...
    with c5:
        ...
        bar_colors_lots = ["#02A3A6" if v >= avg_lots*1.1 else "#C44545" if v < avg_lots*0.7 else "#2384A1" for v in lots_df["Lots"]]
        fig_lots = go.Figure(go.Bar(
            ...
            textfont=dict(size=10, family="JetBrains Mono", color="#113941"),
        ))
        fig_lots.add_hline(y=avg_lots, line_dash="dot", line_color="#02A3A6", line_width=1.5,
    ...
    with c6:
        ...
        fig_scatter.update_traces(textposition="top center", textfont=dict(size=10, color="#113941"))
    ...
    with st.expander("🧵 Quality-wise Breakdown"):
        ...
        with col_a:
            ...
            """, unsafe_allow_html=True)
        with col_b:
            fig_qtree = px.treemap(q_summary.head(30), path=["QUALITY","SHADE"], values="Weight", color="Weight", color_continuous_scale="Teal")
...
<div style="text-align:center;margin-top:32px;color:#9ca3af;font-size:.8rem;border-top:1px solid #e8ecf2;padding-top:16px;">
  ...
  <span style="color:#02A3A6;">● Live</span> · Zoho Creator · Refreshes every {AUTO_REFRESH_SEC}s
</div>
""", unsafe_allow_html=True)